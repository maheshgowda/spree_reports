  ...
  module Reports
    class OrdersByPeriod < SpreeReports::Reports::Base
      ...
      def get_data
        ...
        if @group_by == :year
          @sales = @sales.group_by_year(:created_at,date_column, time_zone: SpreeReports.time_zone).count
        elsif @group_by == :month
          @sales = @sales.group_by_month(:created_at,date_column, time_zone: SpreeReports.time_zone)
        elsif @group_by == :week
          # %W => week start: monday, %U => week start: sunday
          @sales = @sales.group_by_week(:created_at,date_column, time_zone: SpreeReports.time_zone).count
        else
          @sales = @sales.group_by_day(:created_at,date_column, time_zone: SpreeReports.time_zone)
        end
